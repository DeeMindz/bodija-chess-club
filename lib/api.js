...
// Fetch all completed tournaments where this player finished 1st, 2nd, or 3rd
export async function fetchPlayerMedals(playerId) {
    guard();
    // Get all tournament_player rows for this player, joined with tournament info
    const { data, error } = await supabase
        .from('tournament_players')
        .select('points, wins, draws, losses, rating_change, tournaments(id, name, date, format, status)')
        .eq('player_id', playerId)
    ...
    for (const row of (data || [])) {
        ...
        // Fetch full standings for this tournament to determine position
        const { data: standings, error: se } = await supabase
            .from('tournament_players')
            .select('player_id, points, wins, buchholz')
            .eq('tournament_id', t.id)
            .order('points', { ascending: false });
        if (se || !standings) continue;

        const position = standings.findIndex(s => s.player_id === playerId) + 1;
    ...
    medals.sort((a, b) => new Date(b.date) - new Date(a.date));
    return medals;
}
