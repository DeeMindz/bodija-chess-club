...
export async function fetchPlayers() {
    ...
    try {
        ...
        const { data, error } = await supabase.from('players').select('id, player_id, name, fide_rating, bodija_rating, peak_rating, games_played, wins, draws, losses, status, is_guest, photo').eq('is_guest', false).order('bodija_rating', { ascending: false })
