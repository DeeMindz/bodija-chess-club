...
export async function fetchTournamentPairings(tournamentId) {
    try {
        const { data, error } = await supabase
            ...
            .select(`
                *,
                white_player:players!pairings_white_player_id_fkey(name, rating),
                black_player:players!pairings_black_player_id_fkey(name, rating)
            `)
        ...
        return (data || []).map(p => ({
            ...
            isBye: p.is_bye,
            round: p.round_number // Assuming you might want to know which round it belongs to
        }));
    } catch (error) {
...
export async function terminateTournament(tournamentId) {
    try {
        ...
        // 4. Delete the tournament (CASCADE will handle games, pairings, tournament_players, rounds)
