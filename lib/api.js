...
// Sync a single completed round to Supabase so viewers can see live pairings & standings.
// Called after each round is confirmed — idempotent (safe to call multiple times).
export async function syncRoundToDb(tournamentId, roundNumber, pairingRows, standingsRows) {
    guard();

    // 1. Insert round row — ignore if already exists
    let roundId = null;
    ...
    // 2. Upsert pairings for this round (update result if already inserted)
    if (pairingRows && pairingRows.length > 0) {
        const rows = pairingRows.map(p => ({ ...p, round_id: roundId }));
        // Delete existing pairings for this round then re-insert (simpler than true upsert without a unique key)
        await supabase.from('pairings').delete().eq('tournament_id', tournamentId).eq('round_id', roundId);
        const { error: pairErr } = await supabase.from('pairings').insert(rows);
        if (pairErr) throw pairErr;
    }

    // 3. Upsert tournament_players standings (keeps standings live for viewers)
    ...
    // 4. Update tournament current_round so viewers know which round is live
    await supabase.from('tournaments').update({ current_round: roundNumber }).eq('id', tournamentId);
}
