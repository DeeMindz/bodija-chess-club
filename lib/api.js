...
export async function fetchPlayers() {
    ...
    try {
        const { data, error } = await supabase.from('players').select('id, name, bodija_rating, peak_rating, wins, draws, losses, games_played, status, is_guest, photo').order('bodija_rating', { ascending: false })
...
export async function fetchGames(limit = null) {
    ...
    try {
        let query = supabase.from('games').select('id, date, game_date, white_player_id, black_player_id, white_player_name, black_player_name, result, white_rating_change, black_rating_change, tournament_name, round_number, created_at').order('created_at', { ascending: false })
        if (limit) {
