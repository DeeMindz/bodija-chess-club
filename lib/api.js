...
export async function syncRoundToDb(tournamentId, roundNumber, pairingRows, standingsRows) {
    ...
    // 1. Insert round row — reuse if already exists
    let roundId = null;
    ...
    // 2. Delete any existing pairings for this round then insert fresh
    // (handles re-sync if called multiple times for the same round)
    ...
    // 3. Upsert tournament_players standings so viewers see live standings too
