...
export async function fetchGames(limit = null) {
    ...
    try {
        let query = supabase.from('games').select('id, date, white_player_name, black_player_name, result, white_rating_change, black_rating_change, tournament_name, round_number, created_at').order('created_at', { ascending: false })
        if (limit) {
