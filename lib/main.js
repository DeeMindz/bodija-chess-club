...
window.recalcStats = async function() {
    if (!supabase) { console.error('No supabase'); return; }
    console.log('Recalculating player stats from games table...');
    const { data: allGames } = await supabase.from('games').select('white_player_id,black_player_id,result');
    const { data: allPlayers } = await supabase.from('players').select('id,name,bodija_rating,peak_rating');
    if (!allGames || !allPlayers) { console.error('Failed to fetch data'); return; }
    ...
    for (const player of allPlayers) {
        ...
        const { error } = await supabase.from('players').update({ wins, draws, losses, games_played: myGames.length }).eq('id', player.id);
        if (error) console.warn('Failed for', player.name, error.message);
        else console.log(`${player.name}: ${myGames.length} games, ${wins}W/${draws}D/${losses}L`);
    }
...
async function silentRefresh() {
    try {
        ...
        const [playersResp, gamesResp, tournamentsResp] = await Promise.all([
            ...
            fetch(base + 'games?select=id,date,white_player_id,black_player_id,white_player_name,black_player_name,result,white_rating_change,black_rating_change,white_rating_before,black_rating_before,tournament_name,round_number,created_at&order=created_at.desc&limit=2000', { headers }),
        ...
        // Fetch player counts for all tournaments in one query
        try {
            const anonKey2 = import.meta.env.VITE_SUPABASE_ANON_KEY;
            const headers2 = { 'apikey': anonKey2, 'Authorization': 'Bearer ' + anonKey2 };
            const tpResp = await fetch(base + 'tournament_players?select=tournament_id,player_id', { headers: headers2 });
            ...
            if (Array.isArray(tpJson)) {
                ...
                extendedTournaments.forEach(t => {
                    if (countMap[t.id]) t.players = new Array(countMap[t.id]); // length-only stub for card display
                });
            }
        } catch(e) { /* non-critical, cards just show 0 */ }
...
function mapTournamentFromDB(dbTournament) {
    ...
    return {
        ...
        total_rounds: dbTournament.total_rounds || 0,   // numeric — total number of rounds
        rounds: [],                                      // array — populated for active/local tournaments
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Fetch player counts per tournament (for card display)
    try {
        const anonKeyTp = import.meta.env.VITE_SUPABASE_ANON_KEY;
        const headersTp = { 'apikey': anonKeyTp, 'Authorization': 'Bearer ' + anonKeyTp };
        const tpResp = await fetch(getSupabaseUrl() + '/rest/v1/tournament_players?select=tournament_id,player_id', { headers: headersTp });
        const tpJson = await tpResp.json();
        if (Array.isArray(tpJson)) {
            const countMap = {};
            tpJson.forEach(row => {
            ...
            extendedTournaments.forEach(t => {
                if (countMap[t.id]) t.players = new Array(countMap[t.id]);
            });
        }
    } catch(e) { console.warn('[BCC] Tournament player counts failed:', e); }
...
function showSection(sectionId) {
    ...
    if (sectionId === 'tournaments') {
        // If there is an active local tournament open, restore the detail view
        // instead of wiping it with the card list
        const live = window._localTournament || (currentTournament?.status?.toLowerCase() === 'active' ? currentTournament : null);
        if (live) {
            // Restore currentTournament and re-render the detail
...
function renderStats() {
    ...
    // Active tournament card — clickable, shown when a local tournament is in progress
    const existing = document.getElementById('dashboardActiveTournament');
    if (existing) existing.remove();
    ...
    const roundsLeft = (local.total_rounds || 0) - (local.current_round || 1) + 1;
    const progress = local.total_rounds > 0
        ? Math.round(((local.current_round - 1) / local.total_rounds) * 100)
        : 0;

    const card = document.createElement('div');
    card.id = 'dashboardActiveTournament';
    card.style.cssText = 'margin-top:16px;background:linear-gradient(135deg,rgba(240,165,0,0.12),rgba(240,165,0,0.04));border:1px solid rgba(240,165,0,0.4);border-radius:12px;padding:14px 16px;cursor:pointer;transition:all 0.2s;';
    card.onmouseenter = () => { card.style.borderColor = 'rgba(240,165,0,0.8)'; card.style.background = 'linear-gradient(135deg,rgba(240,165,0,0.18),rgba(240,165,0,0.08))'; };
    card.onmouseleave = () => { card.style.borderColor = 'rgba(240,165,0,0.4)'; card.style.background = 'linear-gradient(135deg,rgba(240,165,0,0.12),rgba(240,165,0,0.04))'; };
    card.onclick = () => resumeLocalTournament();
    card.innerHTML = `
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
            <div style="display:flex;align-items:center;gap:8px;">
                <span style="width:8px;height:8px;border-radius:50%;background:#4CAF50;display:inline-block;box-shadow:0 0 6px #4CAF50;animation:pulse 1.5s infinite;"></span>
                <span style="color:var(--accent-gold);font-weight:700;font-size:13px;">LIVE</span>
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--accent-gold)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
        </div>
        <div style="font-weight:700;font-size:15px;color:var(--text-primary);margin-bottom:3px;">${local.name}</div>
        <div style="font-size:12px;color:var(--text-secondary);margin-bottom:10px;">Round ${local.current_round} of ${local.total_rounds} &nbsp;·&nbsp; ${local.players?.length || 0} players</div>
        <div style="background:var(--bg-tertiary);border-radius:4px;height:4px;overflow:hidden;">
            <div style="height:100%;width:${progress}%;background:var(--accent-gold);border-radius:4px;transition:width 0.3s;"></div>
        </div>
        <div style="font-size:11px;color:var(--text-secondary);margin-top:6px;text-align:right;">${progress}% complete · tap to resume</div>
    `;
...
function renderTournamentCard(tournament) {
    ...
    if (tournament.status === 'active') {
        progress = `<div class="tournament-stat" ><span class="tournament-stat-value">${roundsPlayed}/${tournament.total_rounds || '?'}</span><span class="tournament-stat-label">Rounds</span></div > `;
    ...
    return `
            <div class="tournament-card fade-in" onclick = "${(tournament.status || 'draft').toLowerCase() === 'draft' ? `editTournament('${tournament?.id ?? ''}')` : `openTournamentDetail('${tournament?.id ?? ''}')`}" style = "cursor: pointer;" >
                    ...
                    <div class="tournament-stats">
                        ...
                        <div class="tournament-stat">
                            <span class="tournament-stat-value">${tournament.total_rounds || '?'}</span>
...
function showTournamentStartPreview() {
    ...
    if (normFmt === 'roundrobin') {
        ...
        // Swiss
        expectedRounds = currentTournament.total_rounds || currentTournament.totalRounds || Math.ceil(Math.log2(n));
...
async function confirmStartTournament() {
    ...
    try {
        ...
        if (normalizeFormat(_format) === 'roundrobin') {
            ...
            // Swiss: use manually entered value, fall back to ceil(log2(n))
            _totalRounds = currentTournament.total_rounds || currentTournament.totalRounds || Math.ceil(Math.log2(_playerCount));
        }
...
async function openTournamentDetail(tournamentId, forceTab = null, forceRound = null) {
    ...
    if (isCompleted) {
        ...
        try {
            ...
            // Map tournament_players → players array
            if (tpRes.data && tpRes.data.length > 0) {
                currentTournament.players = tpRes.data.map(tp => ({
                    id: tp.player_id,
            ...
            // Map pairings → flat array with .round number
            if (pairingsRes.data && pairingsRes.data.length > 0) {
                currentTournament.pairings = pairingsRes.data.map(p => {
                    const roundNum = p.rounds?.round_number || 1;
                    const whitePlayer = currentTournament.players.find(x => x.id === p.white_player_id);
                    const blackPlayer = currentTournament.players.find(x => x.id === p.black_player_id);
                    return {
                        id: p.id,
                        round: roundNum,
                        white: p.white_player_id,
                        whiteName: whitePlayer?.name || p.white_player_id,
                        black: p.black_player_id,
                        blackName: p.is_bye ? 'BYE' : (blackPlayer?.name || p.black_player_id),
                        result: p.result,
...
async function confirmRoundSubmit() {
    ...
    try {
        ...
        if (isLastRound) {
            ...
            // Hide overlay and close confirm modal FIRST before anything else
            hideLoadingModal();
            if (modal) modal.classList.remove('active');
            showToast('Tournament complete! All results synced to database.', 'success');

            // Refresh data silently in background — don't await, don't block navigation
            silentRefresh().catch(e => console.warn('Background refresh failed:', e));

            // Navigate back to tournaments list immediately
...
async function _syncTournamentToSupabase(local) {
    ...
    // Check if rounds already exist for this tournament (retry scenario)
    const { data: existingRounds } = await supabase
        .from('rounds').select('id,round_number').eq('tournament_id', tid);
    let savedRounds = existingRounds || [];
    if (!existingRounds || existingRounds.length === 0) {
        const { data: sr, error: re } = await supabase.from('rounds').insert(roundInserts).select();
        if (re) throw new Error('Rounds insert: ' + re.message);
        savedRounds = sr || [];
    } else {
        console.log('Rounds already exist, skipping insert');
    }
    ...
    // Fetch ALL games for each player from the games table (ground truth)
    // This is accurate even on retry — no fragile incremental accumulation
    ...
    for (const p of local.players) {
        ...
        const globalLosses = myGames.filter(g =>
            ...
            (g.black_player_id === p.id && g.result === '1-0')
        ).length;
    ...
    // 7. Batch h2h: aggregate all games first, then fetch existing rows, then upsert in one call
    const h2hMap = {}; // key: "p1id|p2id" (sorted)
    for (const p of (local.pairings || []).filter(x => !x.isBye && x.result)) {
        const [p1id, p2id] = [p.white, p.black].sort();
        const key = p1id + '|' + p2id;
        const p1IsWhite = p.white === p1id;
        if (!h2hMap[key]) h2hMap[key] = { player1_id: p1id, player2_id: p2id, player1_wins: 0, player2_wins: 0, draws: 0, total_games: 0 };
        h2hMap[key].total_games++;
    ...
    if (h2hEntries.length > 0) {
        ...
        const playerIds = [...new Set(h2hEntries.flatMap(e => [e.player1_id, e.player2_id]))];
        const { data: existingH2H } = await supabase.from('head_to_head')
            .select('*')
        ...
        for (const entry of h2hEntries) {
            const existing = (existingH2H || []).find(r =>
                r.player1_id === entry.player1_id && r.player2_id === entry.player2_id
            );
            if (existing) {
                toUpdate.push({
                    ...
                    total_games: (existing.total_games || 0) + entry.total_games
                });
            } else {
                toInsert.push(entry);
            }
        ...
        // Insert new rows in one call
        if (toInsert.length > 0) {
            const { error: h2hInsertErr } = await supabase.from('head_to_head').insert(toInsert);
            if (h2hInsertErr) console.warn('H2H insert error:', h2hInsertErr.message);
        }

        // Update existing rows — each needs individual update (no bulk update in Supabase REST)
        // But run them in parallel to avoid sequential blocking
        if (toUpdate.length > 0) {
            await Promise.all(toUpdate.map(u =>
                supabase.from('head_to_head').update({
                    player1_wins: u.player1_wins,
                    player2_wins: u.player2_wins,
                    draws: u.draws,
                    total_games: u.total_games
                }).eq('player1_id', u.player1_id).eq('player2_id', u.player2_id)
            ));
        }
    }
}
