...
async function confirmStartTournament() {
    ...
    try {
        ...
        if (currentTournament.format === 'knockout') {
            ...
            const isDouble = currentTournament.isDoubleRoundRobin;
            if (isDouble) {
                // Double: if N is odd, rounds = N*2, if even, rounds = (N-1)*2
                currentTournament.rounds = n % 2 === 0 ? (n - 1) * 2 : n * 2;
            } else {
                // Single: if N is odd, rounds = N, if even, rounds = N-1
...
function updateRoundsInput() {
    ...
    // Check if double round robin is selected
    const isDoubleSelect = document.getElementById('roundRobinType');
    const isDouble = isDoubleSelect && isDoubleSelect.value === 'double';
    ...
    if (format === 'knockout') {
        ...
        // Show auto-calculated rounds based on single/double
        // For 5 players: single = 5 rounds, double = 10 rounds
        // For 4 players: single = 3 rounds, double = 6 rounds
        roundsInput.value = '';
        roundsInput.placeholder = 'Auto';
        roundsInput.disabled = true;
...
function renderPairingCard(pairing, idx) {
    ...
    const resultOptions = `
            ...
            <option value="0.5-0.5" ${pairing.result === '0.5-0.5' ? 'selected' : ''}>½-½ (Draw)</option>
