...
async function confirmGameSubmit() {
    ...
    try {
        // Get updated player objects from the array
        ...
        console.log('Game save result:', savedGame);

        if (savedGame) {
            // Sync local game with database ID
            newGame.id = savedGame.id;
        }

        // Also update player ratings in DB with updated stats
        const whiteUpdate = await api.updatePlayerStats(updatedWhitePlayer);
        const blackUpdate = await api.updatePlayerStats(updatedBlackPlayer);
        console.log('Player updates:', whiteUpdate, blackUpdate);
    } catch (e) {
        console.error("Error saving to Supabase:", e);
        alert("Error saving game. Please check console for details.");
    }
...
function renderTournamentCard(tournament) {
    ...
    if (tournament.status === 'active') {
        ...
        const winner = tournament.standings && tournament.standings.length > 0 ? tournament.standings[0] : null;
    ...
    return `
            <div class="tournament-card fade-in" onclick = "${tournament.status?.toLowerCase() === 'draft' ? `editTournament('${tournament?.id ?? ''}')` : `openTournamentDetail('${tournament?.id ?? ''}')`}" style = "cursor: pointer;" >
...
async function confirmStartTournament() {
    ...
    try {
        ...
        if (roundData) {
            // 4. Add pairings to pairings table
            const pairingsWithRoundId = currentTournament.pairings.map(p => ({
                ...p,
                round_id: roundData.id,
                tournament_id: currentTournament.id
            }));
            await api.addRoundPairings(currentTournament.id, pairingsWithRoundId);
        }
        ...
        console.error("Supabase startTournament failed:", e);
        showToast("Failed to start tournament in database.", 'error');
    }
}
    ...
    if (editId) {
        ...
        if (idx > -1) {
            ...
            try {
                ...
                console.warn("Could not update Supabase, local state updated only.");
            }
        }
        ...
        // This is a simple check, in a real app we might use a more robust way to track the "current" draft
        ...
        if (existingDraft) {
            ...
            try {
                ...
                console.warn("Could not save to Supabase, local state updated only.");
            }
                ...
                <input type="text" id="newPlayerName" placeholder="New player name">
                    ...
                    <div class="selection-summary">
                        ...
                        <div style="display: flex; gap: 12px;">
                            <button class="btn-secondary" onclick="saveTournamentPlayers()">Save Changes</button>
...
async function openTournamentDetail(tournamentId) {
    ...
    // Fetch fresh standings from DB
    try {
        const dbStandings = await api.fetchTournamentStandings(tournamentId);
        if (dbStandings && dbStandings.length > 0) {
            currentTournament.players = dbStandings;
        }
    } catch (e) {
        console.warn("Failed to fetch standings from DB, using local data");
    }
...
function renderTournamentDetail() {
    const grid = document.getElementById('tournamentsGrid');
    const statusClass = currentTournament.status;
    const statusLabel = currentTournament.status.charAt(0).toUpperCase() + currentTournament.status.slice(1);
    const roundsPlayed = Math.max(...currentTournament.pairings.map(p => p.round || 0), 0);
            ...
            <div class="standings-container">
                <div class="standings-table">
                    <div class="standings-header">
                        ...
                        <span class="col-buchholz">Buchholz</span>
