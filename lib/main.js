...
function renderStats() {
    ...
    card.onclick = () => {
        // View-only: navigate to tournaments section and open the detail
        showSection('tournaments');
        document.querySelectorAll('.nav-item').forEach(i => i.classList.remove('active'));
        document.querySelector('.nav-item[data-section="tournaments"]')?.classList.add('active');
        if (window._localTournament) {
            ...
            currentTournamentTab = 'pairings';
            currentViewingRound = window._localTournament.current_round || 1;
            renderTournamentDetail();
        }
    };
    card.innerHTML = `
...
function renderTournaments() {
    ...
    const tabList = [
        { key: 'active',    label: 'Active',    dot: '#ef4444' },
    ...
    const tabsHtml = tabList.map(t => `
        <button class="t-status-tab ${currentTab === t.key ? 'active' : ''}"
                onclick="switchTournamentTab('${t.key}')"
                data-tab="${t.key}">
            <span class="t-tab-dot" style="background:${t.dot};${t.key === 'active' ? 'animation:blink 1.2s infinite;' : ''}"></span>
            ${t.label}
...
function renderTournamentCard(tournament) {
    ...
    // Status dot
    const dotColor = status === 'active' ? '#ef4444' : status === 'draft' ? '#6b7280' : '#22c55e';
    const dotAnim = status === 'active' ? 'animation:blink 1.2s infinite;' : '';
    const dotHtml = `<span class="t-card-dot" style="background:${dotColor};${dotAnim}"></span>`;
...
async function _syncTournamentToSupabase(local) {
    ...
    // ── STEP 7: Update global player stats from games table ──────────────────
