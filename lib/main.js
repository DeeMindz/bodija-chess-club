...
async function confirmStartTournament() {
    ...
    try {
        ...
        if (existingId) {
            // Update the existing draft tournament to Active
            ...
            // No existing tournament — insert new one
        ...
        // Silent background refresh of tournaments list
        await silentRefresh();
...
function generateRoundLocally() {
    ...
    if (format === 'swiss' || format === 'swiss system') {
        rawPairings = generateSwissPairings(players, currentRound);
    } else if (format === 'roundrobin' || format === 'round robin (single)' || format === 'round robin (double)') {
        rawPairings = generateRoundRobinPairings(players, currentRound);
    } else if (format === 'knockout') {
        rawPairings = generateKnockoutPairings(players, currentRound);
    }

    // Tag each pairing with round number (required by renderTournamentPairings)
...
function recordResult(pairingId, result) {
    ...
    // Sync pairings back to current round in rounds array
    if (local.rounds && local.current_round) {
        const roundIdx = local.current_round - 1;
        if (local.rounds[roundIdx]) {
            local.rounds[roundIdx].pairings = local.pairings;
        }
    }
