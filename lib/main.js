...
window.approvePlayerRequest = async function (id, name, email, phone, photo) {
    ...
    try {
        ...
        // Get next player ID number via api
        const existingPlayers = await api.getLastPlayerIdNumber().catch(() => []);
        let nextNum = 1;
        if (existingPlayers && existingPlayers.length > 0) {
            const match = existingPlayers[0].player_id?.match(/BCC(\d+)/);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        players = playersRaw.map(mapPlayerFromDB).filter(p => p !== null);
        games   = gamesRaw.map(mapGameFromDB).filter(g => g !== null);
        console.log('[BCC] Dashboard data ready — players:', players.length, 'games:', games.length);
...
window.adminUpdatePlayerPhoto = async function(playerId, inputEl) {
    ...
    try {
        const base64 = await _compressImage(file, 400, 0.82);

        await api.updatePlayerPhoto(playerId, base64);

        // Update local cache immediately so re-render shows new photo
        const idx = players.findIndex(p => p.id === playerId);
        if (idx !== -1) players[idx].photo = base64;
...
// Compress image file → base64 jpeg, max maxSize px on longest side
