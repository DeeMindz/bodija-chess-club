...
async function openTournamentDetail(tournamentId, forceTab = null, forceRound = null) {
    ...
    if (isCompleted) {
        ...
        // ── ACTIVE (no local copy) — fetch live pairings+standings from DB ──
        // This lets any visitor (non-admin, other devices) view the live tournament
        ...
        try {
            ...
            // Derive current_round from max pairing round
            if (!currentTournament.current_round || currentTournament.current_round === 0) {
                const maxRound = Math.max(...(currentTournament.pairings || []).map(p => p.round || 0), 1);
