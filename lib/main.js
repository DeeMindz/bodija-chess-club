...
window.approvePlayerRequest = async function (id, name, email, phone, photo) {
    showLoadingModal('Approving player...');
    try {
        // Get the next player_id number
        const { data: existingPlayers, error: countError } = await supabase
            .from('players')
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Fetch from Supabase in parallel with timeout handling
    let dbPlayers = [];
    let dbGames = [];
    let dbTournaments = [];

    // Helper function to add timeout to fetch
    function fetchWithTimeout(promise, label, timeoutMs = 8000) {
        return Promise.race([
            promise,
    ...
    try {
        const [playersResult, gamesResult, tournamentsResult] = await Promise.all([
            ...
            fetchWithTimeout(
                supabase.from('games').select('id, date, white_player_id, black_player_id, white_player_name, black_player_name, result, white_rating_change, black_rating_change, tournament_name, round_number, created_at').order('created_at', { ascending: false }).limit(10),
                'Games fetch'
            ),
            fetchWithTimeout(
        ...
        dbGames = gamesResult.data || [];
        dbTournaments = tournamentsResult.data || [];

        console.log('[BCC] Players loaded:', dbPlayers.length);
        console.log('[BCC] Games loaded:', dbGames.length);
        console.log('[BCC] Tournaments loaded:', dbTournaments.length);
    } catch (e) {
        console.error('[BCC] Parallel fetch failed:', e.message, e);
    }
    ...
    console.log('[BCC] Dashboard rendered');
    console.timeEnd('[BCC] Total load time');
