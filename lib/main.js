...
async function checkAuthSession() {
    try {
        ...
        if (session) {
            ...
            // console.log('Admin session found:', adminEmail);
        }
    } catch (e) {
...
function initAuthListener() {
    supabase.auth.onAuthStateChange((event, session) => {
        if (event === 'SIGNED_IN' && session) {
            isAdmin = true;
            adminEmail = session.user.email || '';
            // console.log('Admin signed in:', adminEmail);
...
function updateAdminUI() {
    // console.log('updateAdminUI called, isAdmin:', isAdmin);
    ...
    // console.log('Body has is-admin class:', document.body.classList.contains('is-admin'));
...
window.openAdminLoginModal = function () {
    // console.log('openAdminLoginModal called');
...
window.submitAdminLogin = async function (event) {
    // console.log('submitAdminLogin called');
...
window.openPlayerRequestModal = function () {
    // console.log('openPlayerRequestModal called');
...
window.submitPlayerRequest = async function (event) {
    ...
    try {
        ...
        if (error) {
            // console.log('Player request saved locally (table may not exist):', error.message);
        }
    } catch (e) {
        // console.log('Player request submitted (offline mode):', e);
    }
    ...
    try {
        ...
        if (!hasStatusColumn) {
            try {
                ...
                console.log('[BCC] Player requests table might not exist');
            }
        }
        ...
        if (pendingPlayerRequests.length === 0) {
            container.innerHTML = '<div style="text-align: center; padding: 20px; color: var(--text-secondary);">No pending requests</div>';
        ...
        container.innerHTML = pendingPlayerRequests.map(request => `
        ...
        container.innerHTML = '<div style="text-align: center; padding: 20px; color: var(--text-secondary);">Error loading requests</div>';
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    console.time('[BCC] Total load time');

    console.log('[BCC] T1 - Before auth check:', Date.now());
    await checkAuthSession();
    initAuthListener();
    checkRLSStatus();
    console.log('[BCC] T2 - After auth check:', Date.now());
    ...
    console.log('[BCC] T3 - Before Promise.all fetches:', Date.now());

    try {
        console.log('[BCC] Fetching players...');
        const playersResult = await api.fetchPlayers();
        dbPlayers = playersResult || [];
        console.log('[BCC] Players loaded:', dbPlayers.length, dbPlayers);
    } catch (e) {
        console.error('[BCC] Supabase error:', e.message, e);
        console.warn('Failed to fetch players:', e);
    }

    try {
        console.log('[BCC] Fetching games...');
        const gamesResult = await api.fetchRecentGames();
        dbGames = gamesResult || [];
        console.log('[BCC] Games loaded:', dbGames.length, dbGames);
    } catch (e) {
        console.error('[BCC] Supabase error:', e.message, e);
    ...
    try {
        ...
        console.log('[BCC] Tournaments loaded:', dbTournaments.length, dbTournaments);
    } catch (e) {
        console.error('[BCC] Supabase error:', e.message, e);
        console.warn('Failed to fetch tournaments:', e);
    }

    console.log('[BCC] T4 - After all fetches:', Date.now());
    ...
    console.log('[BCC] T5 - Before dashboard render:', Date.now());
    ...
    console.log('[BCC] T6 - After dashboard render:', Date.now());

    console.log('[BCC] T7 - Before realtime subscription setup:', Date.now());
    ...
    console.log('[BCC] T8 - After realtime subscription setup:', Date.now());

    console.log('[BCC] T9 - Before any remaining init:', Date.now());
    ...
    console.log('[BCC] T10 - Init complete:', Date.now());
});
...
function showSection(sectionId) {
    console.log('[BCC] Nav clicked:', sectionId);
    console.log('[BCC] Data available at nav click - players:', players?.length, 'games:', games?.length);
...
function populateH2HSelects() {
    ...
    sel1.innerHTML = '<option value="">Player 1</option>' + opts;
    sel2.innerHTML = '<option value="">Player 2</option>' + opts;
}
...
function renderTournaments() {
    ...
    // Get filter values
    const activeTab = document.querySelector('#tournamentStatusTabs .round-pill.active');
        ...
        showConfirmModal(
            ...
            async () => {
                ...
                try {
                    if (btn) {
                        btn.disabled = true;
                        btn.innerHTML = `<span class="spinner-sm"></span> Terminating...`;
                    }
...
function generateSwissPairings(players, round) {
    // Sort players by points (primary) and buchholz (secondary)
    players.sort((a, b) => {
...
function generateRoundRobinPairings(players, round) {
    ...
    const r = round - 1; // 0-based index for rotation logic
    const pairings = [];

    // Correct Round Robin rotation for a specific round
    // Round 1 (r=0) is the initial orientation.
    // For each round 'r', we rotate playersList except the first one 'r' times.
...
function setupSearchFilters() {
    ...
    const statusTabs = document.querySelectorAll('#tournamentStatusTabs .round-pill');
