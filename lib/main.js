...
setTimeout(() => {
    ...
    if (loadingScreen && loadingScreen.style.display !== 'none') {
        // Cinematic exit — fade out after 3.8s min so animation plays, or immediately if overdue
        const minShow = 3800;
...
function openFirstActiveTournament() {
    // Local tournament takes priority
    ...
    // Otherwise open first DB-active tournament
    const first = extendedTournaments.find(t => t.status?.toLowerCase() === 'active');
    if (first) openTournamentDetail(first.id);
}
