...
function showLoadingModal(message = 'Saving...') {
    // Safety: auto-hide after 15s to prevent permanent stuck state
    if (_loadingModalTimeout) clearTimeout(_loadingModalTimeout);
    _loadingModalTimeout = setTimeout(() => {
        console.warn('[BCC] Loading modal auto-dismissed after 15s timeout');
        hideLoadingModal();
    }, 15000);
...
function hideLoadingModal() {
    // Always force-hide — no counter, no stuck states
...
async function openTournamentDetail(tournamentId, forceTab = null, forceRound = null) {
    ...
    if (isCompleted) {
        ...
        try {
            ...
            if (rawPairings && rawPairings.length > 0) {
                // Pairings exist in DB — tournament has completed at least one synced round
                currentTournament.pairings = rawPairings;
                ...
                // Set a flag so renderTournamentDetail can show a friendly message
...
function openFirstActiveTournament() {
    ...
    // Open the tournament detail modal directly — works on mobile without needing section navigation
    const modal = document.getElementById('tournamentDetailModal');
    if (modal) modal.classList.add('active');
