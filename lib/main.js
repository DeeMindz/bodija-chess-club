...
function mapPlayerFromDB(dbPlayer) {
    ...
    // Use games_played from DB but fall back to sum of W+D+L if it looks wrong (0 when W/D/L > 0)
...
function renderStats() {
    ...
    // Active tournament section — uses dedicated HTML slot, works on all screen sizes
    _renderActiveTournamentCards();
}

function _renderActiveTournamentCards() {
    const slot = document.getElementById('dashboardActiveTournamentSlot');
    if (!slot) return;

    // Collect active tournaments — local takes priority, then DB active ones
    const activeTournaments = extendedTournaments.filter(t =>
        t.status?.toLowerCase() === 'active'
    );
    const local = window._localTournament;

    // Nothing to show
    if (!local && activeTournaments.length === 0) {
    ...
    // Build list — local tournament first (if running), then other DB actives
    const items = [];
    if (local && !local.synced) {
        items.push({ t: local, isLocal: true });
    }
    activeTournaments.forEach(t => {
        if (!local || t.id !== local.id) items.push({ t, isLocal: false });
    });
    ...
    const sectionTitle = `
        <h3 style="margin:0 0 12px;font-size:14px;font-weight:600;color:var(--text-secondary);
            ...
            Active Tournaments
            <span style="background:var(--success);color:#000;border-radius:10px;padding:1px 7px;font-size:11px;font-weight:700;">${items.length}</span>
        </h3>`;

    const cardsHtml = items.map(({ t, isLocal }) => {
        ...
        return `
            <div onclick="${onclick}"
                 style="background:linear-gradient(135deg,rgba(240,165,0,0.1),rgba(240,165,0,0.03));
                        border:1.5px solid rgba(240,165,0,0.35);border-radius:12px;padding:14px 16px;
                        cursor:pointer;transition:border-color 0.2s;margin-bottom:10px;"
                 onmouseenter="this.style.borderColor='rgba(240,165,0,0.7)'"
                 onmouseleave="this.style.borderColor='rgba(240,165,0,0.35)'">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
                    <div style="display:flex;align-items:center;gap:8px;">
                        <span style="width:8px;height:8px;border-radius:50%;background:#22c55e;display:inline-block;
                                     box-shadow:0 0 6px #22c55e;animation:blink 1.2s infinite;flex-shrink:0;"></span>
                ...
                <div style="font-size:12px;color:var(--text-secondary);margin-bottom:10px;">
                    Round ${t.current_round || 1} of ${t.total_rounds || '?'} &nbsp;·&nbsp; ${playerCount} players
                </div>
                ...
                <div style="font-size:11px;color:var(--text-secondary);margin-top:5px;text-align:right;">
                    ${progress}% complete &nbsp;·&nbsp; tap to view
                </div>
            </div>`;
    }).join('');

    slot.innerHTML = sectionTitle + cardsHtml;
}
