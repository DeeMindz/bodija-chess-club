...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    api.subscribeToAllTables({
        onPlayers: (payload) => {
            // Re-fetch and re-render leaderboard when any player record changes
        ...
        onTournaments: (payload) => {
            // Re-fetch tournaments list when any tournament is created/updated/completed
...
async function confirmRoundSubmit() {
    ...
    try {
        ...
        if (isLastRound) {
            ...
            // Fire-and-forget: don't await, don't block admin flow
            const tid = local.id;
