...
function updateAdminUI() {
    ...
    // Load pending request count if admin
    if (isAdmin) {
...
async function checkRLSStatus() {
    try {
        ...
        console.warn('âš ï¸ Supabase connection issue. Ensure RLS is enabled on your Supabase dashboard.');
    }
}
...
window.sendOTP = async function (event) {
    ...
    try {
        ...
        if (errorEl) errorEl.textContent = 'âœ“ OTP sent to your email!';
...
// Load pending request count for notification badge
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!supabase) {
        ...
        document.body.innerHTML = '<div style="padding:50px;text-align:center;"><h1>âš ï¸ App Not Configured</h1><p>Supabase credentials missing. Check console for details.</p></div>';
        return;
    }

    console.log('[BCC] Fetching data from database...');

    // Fetch sequentially with timeouts
    ...
    try {
        ...
        console.log('[BCC] Players JSON:', JSON.stringify(playersJson).slice(0, 200));
        ...
        const gamesResp = await fetch(getSupabaseUrl() + '/rest/v1/games?select=id,date,white_player_name,black_player_name,result,white_rating_change,black_rating_change,white_rating_before,black_rating_before,tournament_name,round_number,created_at&order=created_at.desc&limit=10', { headers });
...
function formatPoints(val) {
    ...
    if (hasHalf) return whole === 0 ? 'Â½' : `${whole}Â½`;
    return String(whole);
}
...
function renderH2HContent(p1id, p2id) {
    ...
    const filterLabels = [
        ...
        { key: 'rapid', label: 'Rapid', icon: 'âš¡' },
        { key: 'blitz', label: 'Blitz', icon: 'âš¡' },
        { key: 'bullet', label: 'Bullet', icon: 'ðŸ’¨' }
    ];
...
async function confirmGameSubmit() {
    ...
    try {
        ...
        // Refresh data after game save
        games = await api.fetchRecentGames();
...
function renderTournamentCard(tournament) {
    ...
    if (tournament.status === 'active') {
        ...
        progress = winner ? `<div class="tournament-stat"><span class="tournament-stat-value" style="color: var(--accent-gold);">ðŸ† ${winner.name.split(' ')[0]}</span><span class="tournament-stat-label">Winner</span></div>` : '';
    }
...
async function confirmStartTournament() {
    ...
    try {
        ...
        } else if (currentTournament.format === 'Round Robin (Single)' || currentTournament.format === 'Round Robin (Double)') {
            currentTournament.rounds = calculateTotalRounds(currentTournament.format, currentTournament.players.length);
        }
        ...
        const pairingsWithIds = currentTournament.pairings.map(p => ({
            ...p,
            round_id: roundData.id,
            tournament_id: tournamentId
        }));
        await api.addRoundPairings(tournamentId, pairingsWithIds);
...
async function deleteTournament(id) {
    ...
    if (tournament.status?.toLowerCase() === 'active') {
        showConfirmModal(
            ...
            async () => {
                const btn = document.getElementById(`tournament-btn-${id}`);
                const originalHtml = btn ? btn.innerHTML : '';

                try {
                    if (btn) {
                        btn.disabled = true;
                        if (btn) btn.innerHTML = `<span class="spinner-sm"></span> Terminating...`;
                    }
    ...
    showConfirmModal(
        ...
        async () => {
            ...
            try {
                if (btn) {
                    btn.disabled = true;
                    if (btn) btn.innerHTML = `<span class="spinner-sm"></span> Deleting...`;
                }
...
function getColorHistory(tournament) {
    if (!tournament.colorHistory) {
        tournament.colorHistory = {};
    }
    return tournament.colorHistory;
}

// Assign colors based on history to balance white/black
function assignColors(player1, player2, colorHistory) {
    const p1Id = player1.id;
    const p2Id = player2.id;

    const p1Data = colorHistory[p1Id] || { whites: 0, blacks: 0 };
    const p2Data = colorHistory[p2Id] || { whites: 0, blacks: 0 };

    // If this pairing already happened, swap colors from first game
    const pairKey = [p1Id, p2Id].sort().join('-');
    if (colorHistory[pairKey]) {
        // Swap from first game
        return { white: player2, black: player1 };
    }

    // Give white to player with more blacks so far
    if (p1Data.blacks > p1Data.whites) {
        return { white: player1, black: player2 };
    }
    if (p2Data.blacks > p2Data.whites) {
        return { white: player2, black: player1 };
    }

    // Otherwise give white to player with fewer whites
    if (p1Data.whites < p2Data.whites) {
        return { white: player1, black: player2 };
    }
    if (p2Data.whites < p1Data.whites) {
        return { white: player2, black: player1 };
    }

    // Equal - alternate based on round number or random
    return { white: player1, black: player2 };
}

// Record color assignment in history
function recordColors(tournament, whitePlayer, blackPlayer) {
    const colorHistory = getColorHistory(tournament);

    // Update individual player history
    if (!colorHistory[whitePlayer.id]) colorHistory[whitePlayer.id] = { whites: 0, blacks: 0 };
    if (!colorHistory[blackPlayer.id]) colorHistory[blackPlayer.id] = { whites: 0, blacks: 0 };

    colorHistory[whitePlayer.id].whites++;
    colorHistory[blackPlayer.id].blacks++;

    // Record pairing (for double RR to know which colors were used)
    const pairKey = [whitePlayer.id, blackPlayer.id].sort().join('-');
    if (!colorHistory[pairKey]) {
        colorHistory[pairKey] = { whiteId: whitePlayer.id, blackId: blackPlayer.id };
    }
}
...
function generatePairings() {
    ...
    if (format === 'Swiss System') {
        pairings = generateSwissPairings(players, currentTournament.current_round || 1);
    } else if (format === 'Round Robin (Single)' || format === 'Round Robin (Double)') {
...
// Formula: if N is odd, rounds = N. If N is even, rounds = N-1
function generateRoundRobinPairings(players, round) {
    const n = players.length;
    if (n < 2) return [];

    // Check if double round robin
    const isDouble = currentTournament.isDoubleRoundRobin;
    const totalRounds = isDouble ? (n % 2 === 0 ? (n - 1) * 2 : n * 2) : (n % 2 === 0 ? n - 1 : n);
    const halfRounds = isDouble ? Math.ceil(totalRounds / 2) : totalRounds;

    // For double RR, first half uses normal pairing, second half swaps colors
    const isSecondHalf = isDouble && round > halfRounds;
    const actualRound = isDouble && round > halfRounds ? round - halfRounds : round;

    const playersList = [...players];

    // Handle odd number of players - add BYE
    if (n % 2 !== 0) {
        playersList.push({ id: 'BYE', name: 'BYE', rating: 0 });
    }

    const numPlayers = playersList.length;
    const pairings = [];

    // Berger circle method: fix first player, rotate others
    const roundPlayers = [...playersList];
    for (let i = 0; i < actualRound - 1; i++) {
        // Rotate: take last and insert after first
        roundPlayers.splice(1, 0, roundPlayers.pop());
    }

    // Generate pairings - first half of list vs second half (reversed)
    for (let i = 0; i < numPlayers / 2; i++) {
        ...
        let white, black;
        if (isSecondHalf) {
            // In second half of double RR, swap colors
            white = p2;
            black = p1;
        } else {
            // Use Berger: odd positions get white in even rounds, even positions get white in odd rounds
            white = (actualRound + i) % 2 === 0 ? p1 : p2;
            black = (actualRound + i) % 2 === 0 ? p2 : p1;
        }
...
function generateSwissPairings(players, round) {
    ...
    for (let i = 0; i < players.length; i++) {
        ...
        // Find best match - same score group preferred, avoid rematches
        let p2 = null;
        let bestScore = -1;
        ...
        for (let j = i + 1; j < players.length; j++) {
            ...
            // Check for rematch
            const pairKey = [p1.id, candidate.id].sort().join('-');
            const colorHistoryPair = colorHistory[pairKey];
            if (colorHistoryPair) continue; // Already played

            // Prefer same score
            const scoreDiff = Math.abs(candidate.points - p1.points);
            if (scoreDiff < bestScore || bestScore === -1) {
                bestScore = scoreDiff;
                p2 = candidate;
            }
        }

        // If no same-score opponent found, find any available
        if (!p2) {
            for (let j = i + 1; j < players.length; j++) {
...
function generateKnockoutPairings(players, round) {
    // Sort by rating for seeding (highest gets bye if needed)
    if (round === 1) {
    ...
    for (let i = 0; i < players.length; i += 2) {
        ...
        if (p2) {
            ...
            // Odd player gets bye to next round
            pairings.push({
                round: round,
                white: p1.id,
                whiteName: p1.name,
...
// Validate round robin pairings
function validateRoundRobin(players, isDouble) {
    const n = players.length;
    const errors = [];

    // Check total rounds
    const expectedRounds = isDouble
        ? (n % 2 === 0 ? (n - 1) * 2 : n * 2)
        : (n % 2 === 0 ? n - 1 : n);

    // Check all pairings across all rounds
    // Each player should play n-1 games (single) or (n-1)*2 (double)
    const expectedGamesPerPlayer = isDouble ? (n - 1) * 2 : n - 1;

    // Verify pairing uniqueness would require storing all pairings
    // For now, log validation info
    console.log(`Round Robin Validation: ${n} players, ${expectedRounds} rounds, ${expectedGamesPerPlayer} games per player`);
                    ...
                    <div class="tournament-detail-header">
                        <div>
                            ...
                            <div class="tournament-detail-meta">
                                <span>ðŸ“… ${new Date(currentTournament.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                                <span>ðŸŽ¯ ${currentTournament.format === 'swiss' ? 'Swiss System' : currentTournament.format === 'roundrobin' ? 'Round Robin' : 'Knockout'}</span>
                                <span>â±ï¸ ${currentTournament.timeControl}</span>
                                <span>ðŸ‘¥ ${currentTournament.players.length} Players</span>
                                <span class="tournament-status-badge ${statusClass}">${statusLabel}</span>
                            </div>
                        </div>
...
function renderTournamentOverview() {
    ...
    const roundsPlayed = isCompleted ? (currentTournament.rounds || 0) : Math.max(0, (currentTournament.current_round || 1) - 1);
...
function renderTournamentFinalSummary() {
    ...
    return `
            <div class="tournament-complete-banner" style="background: linear-gradient(135deg, var(--accent-gold), #B8860B); padding: 30px; border-radius: 12px; text-align: center; margin-bottom: 30px; color: white;">
                <h2 style="font-size: 28px; margin-bottom: 8px;">Tournament Complete! ðŸ†</h2>
    ...
    for (let r = 1; r <= maxRound; r++) {
        ...
        navHtml += `
            <div class="round-pill ${isActive ? 'active' : ''} ${isCompleted ? 'completed' : ''}" onclick="switchViewingRound(${r})">
                Round ${r} ${isCompleted ? '<span class="check-icon">âœ“</span>' : ''}
            </div>
        `;
    }
    navHtml += '</div>';
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
            ...
            ${isCurrentActiveRound ? `
                <div style="font-size: 14px; color: ${resultsRemaining === 0 ? 'var(--success)' : 'var(--text-secondary)'};">
                    ${resultsRemaining === 0 ? 'âœ“ All results entered' : `ðŸ“ ${resultsRemaining} results remaining`}
                </div>
            ` : ''}
        </div>
        ...
        html += `
            <div class="round-actions-sticky" style="margin-top: 24px; padding: 16px; background: var(--bg-secondary); border-radius: 8px; border: 1px solid var(--border-color); display: flex; justify-content: flex-end; gap: 12px; position: sticky; bottom: 20px; z-index: 100;">
                <button class="btn-primary" id="nextRoundBtn" onclick="generateNextRound()" ${resultsRemaining > 0 ? 'disabled style="opacity: 0.5; cursor: not-allowed;"' : ''}>
                    ${currentViewingRound === currentTournament.rounds ? 'Complete Tournament' : 'Generate Next Round'}
                </button>
            </div>
        `;
    }
...
function renderPairingCard(pairing, idx) {
    ...
    const resultOptions = `
            ...
            <option value="1/2-1/2" ${pairing.result === '1/2-1/2' ? 'selected' : ''}>Â½-Â½ (Draw)</option>
    ...
    return `
            <div class="pairing-card ${isBye ? 'bye-card' : ''}" style="display: grid; grid-template-columns: 1fr auto 1fr auto; align-items: center; gap: 16px; padding: 16px; background: var(--bg-secondary); border-radius: 8px; margin-bottom: 12px; border: 1px solid var(--border-color);">
                    ...
                    <div class="pairing-player black" style="text-align: left;">
                        ...
                        <div class="rating" style="font-size: 12px; color: var(--text-secondary);">Rating: ${pairing.blackRatingBefore}</div>
                    </div>
        ...
        <div class="confirm-results-list" style="margin-bottom: 24px; background: var(--bg-tertiary); border-radius: 8px; overflow: hidden; border: 1px solid var(--border-color);">
            ${pairings.map(p => `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; border-bottom: 1px solid var(--border-color);">
                    ...
                    <span style="font-weight: 700; color: var(--accent-gold); background: var(--bg-secondary); padding: 4px 12px; border-radius: 4px; border: 1px solid var(--border-color);">${p.result || 'BYE'}</span>
                </div>
            `).join('')}
        </div>
    `;
...
async function confirmRoundSubmit() {
    const currentRound = currentTournament.current_round || 1;
    const currentRoundPairings = currentTournament.pairings.filter(p => p.round === currentRound);
    // Be robust: find any pairing that has a roundId/round_id
    const roundId = currentRoundPairings.find(p => p.roundId || p.round_id)?.roundId ||
        currentRoundPairings.find(p => p.roundId || p.round_id)?.round_id;
    ...
    try {
        if (btn) {
            btn.disabled = true;
            btn.textContent = 'Syncing Standings...';
        }
        ...
        for (const pairing of currentRoundPairings) {
            ...
            if (whitePlayer) {
                ...
                if (whitePlayer.rating > (whitePlayer.peakRating || 0)) {
            ...
            if (blackPlayer) {
                ...
                // Update peak rating if new rating is higher
                if (blackPlayer.rating > (blackPlayer.peakRating || 0)) {
        ...
        if (currentRound === currentTournament.rounds) {
            ...
            currentTournamentTab = 'overview';
            // Reload page to refresh all data including leaderboard ratings
            window.location.reload();
        } else {
            ...
            if (nextRoundData) {
                // Update local state for redirection
                currentTournament.current_round = nextRoundNum;
                currentTournamentTab = 'pairings';
                currentViewingRound = nextRoundNum;

                // Generate and save next pairings (format-aware)
                generatePairings();
                const pairingsWithIds = currentTournament.pairings.map(p => ({
                    ...p,
                    roundId: nextRoundData.id,
                    tournament_id: currentTournament.id
                }));

                // Update local pairings with IDs so they are ready if needed before reload
                currentTournament.pairings = pairingsWithIds;
        ...
        // 4. Finalize
        if (roundId) {
            await api.updateRoundStatus(roundId, 'Completed');
        } else {
            console.warn("No roundId found to mark as completed for round", currentRound);
        }

        showToast("Round results saved successfully!", 'success');

        saveTournamentState();
        setTimeout(() => window.location.reload(), 1000);
    } catch (error) {
        console.error("Error confirming round:", error);
        showToast("Failed to sync data to database.", 'error');
        if (btn) {
            btn.disabled = false;
            btn.textContent = originalText;
        }
    ...
    // Refresh data after tournament operations
    games = (await api.fetchRecentGames()).map(mapGameFromDB).filter(g => g !== null);
    players = (await api.fetchPlayers()).map(mapPlayerFromDB).filter(p => p !== null);
    renderGamesLog();
    renderLeaderboard();
    renderDashboard();
