...
async function checkRLSStatus() {
    try {
        ...
        console.warn('⚠️ Supabase connection issue. Ensure RLS is enabled on your Supabase dashboard.');
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!supabase) {
        ...
        document.body.innerHTML = '<div style="padding:50px;text-align:center;"><h1>⚠️ App Not Configured</h1><p>Supabase credentials missing. Check console for details.</p></div>';
        return;
    }
    ...
    console.log('[BCC] Games fetch:', gamesResult.error ? 'ERROR: ' + gamesResult.error.message : 'success, count: ' + (gamesResult.data?.length || 0));
    console.log('[BCC] Tournaments fetch:', tournamentsResult.error ? 'ERROR: ' + tournamentsResult.error.message : 'success, count: ' + (tournamentsResult.data?.length || 0));

    if (playersResult.error) console.error('[BCC] Players error:', playersResult.error);
    if (gamesResult.error) console.error('[BCC] Games error:', gamesResult.error);
    if (tournamentsResult.error) console.error('[BCC] Tournaments error:', tournamentsResult.error);
                    ...
                    <div class="tournament-detail-header">
                        <div>
                            ...
                            <div class="tournament-detail-meta">
                                ...
                                <span>⏱️ ${currentTournament.timeControl}</span>
