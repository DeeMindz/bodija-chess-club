...
async function confirmRoundSubmit() {
    ...
    try {
        ...
        if (isLastRound) {
            ...
            currentTournament = local;
            currentTournament.current_round = currentRound + 1;
            saveLocalTournament();
...
async function _syncTournamentToSupabase(local) {
    ...
    const { data: savedRounds, error: re } = await supabase.from('rounds').insert(roundInserts).select();
    ...
    const { data: savedPairings, error: pe } = await supabase.from('pairings').insert(pairingInserts).select();
    ...
    for (const p of (local.pairings || []).filter(x => !x.isBye && x.result)) {
        ...
        const p1wins = p.result === (p.white === p1id ? '1-0' : '0-1') ? 1 : 0;
        const p2wins = p.result === (p.black === p2id ? '0-1' : '1-0') ? 1 : 0;
        const draws = p.result === '1/2-1/2' ? 1 : 0;
        const { data: existing } = await supabase.from('head_to_head')
            .select('*').eq('player1_id', p1id).eq('player2_id', p2id).single();
        if (existing) {
