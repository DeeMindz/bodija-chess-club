...
function renderTournamentFinalSummary() {
    ...
    return `
            <div class="tournament-complete-banner" style="
                background: linear-gradient(135deg, rgba(240,165,0,0.13), rgba(184,134,11,0.07));
                ...
                <!-- Subtle radial glow -->
                <div style="position:absolute;inset:0;background:radial-gradient(ellipse at 50% 0%, rgba(240,165,0,0.18) 0%, transparent 70%);pointer-events:none;"></div>

                <!-- SVG trophy character -->
                <div style="margin-bottom: 12px; line-height: 1;">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 140" width="110" height="128" style="display:inline-block;">
                        <!-- Trophy cup body -->
                        <path d="M35 18 Q30 18 28 24 Q20 24 18 32 Q16 44 24 52 Q30 58 38 60 Q42 74 50 80 L50 96 L40 100 Q36 102 36 108 L84 108 Q84 102 80 100 L70 96 L70 80 Q78 74 82 60 Q90 58 96 52 Q104 44 102 32 Q100 24 92 24 Q90 18 85 18 Z" fill="var(--accent-gold)" opacity="0.95"/>
                        <!-- Trophy handles -->
                        <path d="M28 24 Q14 26 14 40 Q14 52 26 56" fill="none" stroke="var(--accent-gold)" stroke-width="5" stroke-linecap="round" opacity="0.8"/>
                        <path d="M92 24 Q106 26 106 40 Q106 52 94 56" fill="none" stroke="var(--accent-gold)" stroke-width="5" stroke-linecap="round" opacity="0.8"/>
                        <!-- Shine on cup -->
                        <path d="M44 26 Q42 38 44 50" stroke="rgba(255,255,255,0.35)" stroke-width="3.5" stroke-linecap="round" fill="none"/>
                        <path d="M50 22 Q49 30 50 38" stroke="rgba(255,255,255,0.2)" stroke-width="2" stroke-linecap="round" fill="none"/>
                        <!-- Base plate -->
                        <rect x="32" y="108" width="56" height="7" rx="3.5" fill="var(--accent-gold)" opacity="0.85"/>
                        <rect x="26" y="115" width="68" height="8" rx="4" fill="var(--accent-gold)" opacity="0.7"/>
                        <!-- Star sparkles -->
                        <g fill="var(--accent-gold)" opacity="0.6">
                            <polygon points="12,14 13.5,18 17,18 14.5,20.5 15.5,24 12,22 8.5,24 9.5,20.5 7,18 10.5,18" transform="scale(0.7) translate(6,2)"/>
                            <polygon points="108,8 109,11 112,11 110,13 110.5,16 108,14.5 105.5,16 106,13 104,11 107,11" transform="scale(0.65) translate(-8,4)"/>
                            <circle cx="18" cy="88" r="2.5" opacity="0.4"/>
                            <circle cx="102" cy="80" r="2" opacity="0.35"/>
                        </g>
                        <!-- Tiny confetti dots -->
                        <circle cx="22" cy="30" r="2" fill="#FFD700" opacity="0.5"/>
                        <circle cx="98" cy="35" r="1.8" fill="#FFD700" opacity="0.45"/>
                        <circle cx="15" cy="70" r="1.5" fill="var(--accent-gold)" opacity="0.4"/>
                        <circle cx="105" cy="65" r="1.5" fill="var(--accent-gold)" opacity="0.4"/>
                    </svg>
                </div>

                <!-- Winner name above congratulations text -->
                <div style="font-size: 22px; font-weight: 800; color: var(--accent-gold); letter-spacing: 0.3px; margin-bottom: 4px;">
                    \${winner?.name || 'Champion'}
                </div>
                <div style="font-size: 13px; color: var(--text-secondary); letter-spacing: 1.5px; text-transform: uppercase; margin-bottom: 10px; font-weight: 600;">
                    Tournament Champion
                </div>
                <div style="font-size: 13px; color: var(--text-secondary); opacity: 0.8;">
                    \${currentTournament.name} &nbsp;&middot;&nbsp; \${new Date(currentTournament.date).toLocaleDateString('en-US', { year:'numeric', month:'short', day:'numeric' })}
                </div>
            </div>
...
async function confirmRoundSubmit() {
    ...
    try {
        ...
        if (isLastRound) {
            ...
            // Close both modals and navigate to tournaments page
            hideLoadingModal();
            if (modal) modal.classList.remove('active');
            ...
            // Await refresh so completed tournament appears immediately, then navigate
