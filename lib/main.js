...
window.submitPlayerRequest = async function (event) {
    ...
    try {
        ...
        const { error } = await supabase.from('player_requests').insert([
            { name, email, phone, experience, status: 'pending', created_at: new Date().toISOString() }
        ]);

        if (error) {
            console.log('Player request saved locally (table may not exist)');
        }
    } catch (e) {
        console.log('Player request submitted (offline mode)');
    }
