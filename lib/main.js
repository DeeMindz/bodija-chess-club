...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // Populate medals cache (non-blocking)
        api.fetchAllPlayersMedals().then(m => { medalsCache = m; renderLeaderboard(); renderPlayers(); }).catch(e => console.warn('Medals initial load failed:', e));
        tournamentsRaw.forEach(t => {
            const mapped = mapTournamentFromDB(t);
            if (mapped) {
                if (tpCountMap[mapped.id]) { mapped.players = new Array(tpCountMap[mapped.id]); mapped.playerCount = tpCountMap[mapped.id]; }
