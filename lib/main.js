...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Check for local tournament FIRST - before any Supabase fetch
...
function generatePairings() {
    ...
    if (format === 'swiss' || format === 'Swiss System') {
        pairings = generateSwissPairings(players, currentTournament.current_round || 1);
    } else if (format === 'roundrobin' || format === 'Round Robin (Single)' || format === 'Round Robin (Double)') {
        pairings = generateRoundRobinPairings(players, currentTournament.current_round || 1);
    } else if (format === 'knockout' || format === 'Knockout') {
...
// Generate round locally and save to localStorage (for offline tournament)
function generateRoundLocally() {
    ...
    if (format === 'swiss' || format === 'Swiss System') {
        pairings = generateSwissPairings(players, currentRound);
    } else if (format === 'roundrobin' || format === 'Round Robin (Single)' || format === 'Round Robin (Double)') {
        pairings = generateRoundRobinPairings(players, currentRound);
    } else if (format === 'knockout' || format === 'Knockout') {
        pairings = generateKnockoutPairings(players, currentRound);
    }
    ...
    // Push to local tournament rounds
    currentTournament.rounds.push(round);

    // Set current pairings
    currentTournament.pairings = pairings;

    // Update window._localTournament if it exists
    if (window._localTournament) {
        window._localTournament.rounds = currentTournament.rounds;
        window._localTournament.pairings = pairings;
        saveLocalTournament();
    }

    console.log('Generated round locally:', round);
}
...
async function confirmRoundSubmit() {
    ...
    try {
        ...
        showLoadingModal('Processing round results...');
        ...
        for (const pairing of currentRoundPairings) {
            ...
            const whiteElo = calculateElo(pairing.whiteRatingBefore, pairing.blackRatingBefore, resultValue);
            const blackElo = calculateElo(pairing.blackRatingBefore, pairing.whiteRatingBefore, 1 - resultValue);
