...
async function openTournamentDetail(tournamentId) {
    ...
    // Default to pairings tab for active tournaments
    if (currentTournament.status?.toLowerCase() === 'active') {
        currentTournamentTab = 'pairings';
    } else {
    ...
    return `
            <div class="pairing-card ${isBye ? 'bye-card' : ''}" style="display: grid; grid-template-columns: 1fr auto 1fr auto; align-items: center; gap: 16px; padding: 16px; background: var(--bg-secondary); border-radius: 8px; margin-bottom: 12px; border: 1px solid var(--border-color);">
                    ...
                    <div class="pairing-result">
                        ...
                        ` : `
                            <div style="padding: 8px 16px; background: var(--bg-tertiary); border: 1px solid var(--border-color); color: var(--accent-gold); border-radius: 4px; font-weight: 700; text-align: center; min-width: 100px;">
...
async function confirmRoundSubmit() {
    const currentRound = currentTournament.current_round || 1;
    const currentRoundPairings = currentTournament.pairings.filter(p => p.round === currentRound);
    const roundId = currentRoundPairings[0]?.round_id;
    ...
    try {
        ...
        // Advance current viewing round if possible
        if (currentTournament.current_round < currentTournament.rounds) {
            currentViewingRound = currentTournament.current_round + 1;
        }

        showToast("Round results saved successfully!", 'success');

        // Use a small delay before reload to let toast be seen, or skip reload if possible
        // For now, reload is safest for full state consistency
