    ...
    try {
        ...
        try {
            const { data: result, error } = await supabase
                .from('player_requests')
                .select('id, name, email, phone, bodija_rating, fide_rating, photo, status, created_at')
                .eq('status', 'pending')
...
function showSection(sectionId) {
    ...
    console.log('[BCC] Data available at nav click - players:', players?.length, 'games:', dbGames?.length);
