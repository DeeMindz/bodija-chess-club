...
async function confirmRoundSubmit() {
    ...
    try {
        ...
        // Debug: log player ratings before and after recalc
        console.log('[BCC Recalc] Before recalc:', _localForRecalc.players.map(p => ({name: p.name, currentRating: p.currentRating, ratingAtStart: p.ratingAtStart})));
        console.log('[BCC Recalc] Rounds going in:', _localForRecalc.rounds.map(r => ({rn: r.roundNumber, pairings: r.pairings.length, withResults: r.pairings.filter(p=>p.result).length})));
        recalculateFromRound(_localForRecalc);
        console.log('[BCC Recalc] After recalc:', _localForRecalc.players.map(p => ({name: p.name, currentRating: p.currentRating})));
        ...
        if (isLastRound) {
            ...
            // Close ALL modals immediately — use querySelectorAll to catch any
            // that may have re-opened during the async sync steps
