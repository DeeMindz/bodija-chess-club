...
async function loadTournamentState() {
    ...
    if (savedId) {
        ...
        currentTournamentTab = savedTab || 'overview';
        currentViewingRound = savedRound ? parseInt(savedRound) : null;

        // Clear it so it doesn't keep opening on every fresh load
        clearTournamentState();

        await openTournamentDetail(savedId);
    }
}
...
async function confirmRoundSubmit() {
    const currentRound = currentTournament.current_round || 1;
    const currentRoundPairings = currentTournament.pairings.filter(p => p.round === currentRound);
    const roundId = currentRoundPairings[0]?.roundId || currentRoundPairings[0]?.round_id;
