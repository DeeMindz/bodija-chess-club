...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Fetch from Supabase with error handling - run all in parallel
    let dbPlayers = [];
    let dbGames = [];
    let dbTournaments = [];
    ...
    const [playersResult, gamesResult, tournamentsResult] = await Promise.all([
        api.fetchPlayers(),
        api.fetchRecentGames(),
        api.fetchTournaments()
    ]).catch(errors => {
        console.error('[BCC] Fetch errors:', errors);
        return [[], [], []];
    });
    ...
    console.log('[BCC] Players:', dbPlayers.length);
    console.log('[BCC] Games:', dbGames.length, dbGames.error?.message);
    console.log('[BCC] Tournaments:', dbTournaments.length);
