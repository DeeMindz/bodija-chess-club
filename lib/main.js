...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    console.log('[BCC] Tournaments:', tournaments.length, tournamentsResult.error?.message)

    // Map data to global variables
    players = players.map(mapPlayerFromDB).filter(p => p !== null);
    games = games.map(mapGameFromDB).filter(g => g !== null);
