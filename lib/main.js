...
window.recalcStats = async function() {
    ...
    try {
        // Use pairings table — covers ALL tournaments including those where games table is empty
        const [allPairings, allPlayers] = await Promise.all([
            api.fetchAllPairingResults(),
            api.fetchAllPlayersForRecalc()
        ]);
        console.log(`Found ${allPairings.length} completed pairings, ${allPlayers.length} players`);
        for (const player of allPlayers) {
            const myGames = allPairings.filter(g => g.white_player_id === player.id || g.black_player_id === player.id);
...
window.approvePlayerRequest = async function (id, name, email, phone, photo) {
    try {
        ...
        try {
            ...
            showToast('Error adding player: ' + createErr.message, 'error');
            return;
        }
...
async function silentRefresh() {
    try {
        ...
        tournamentsRaw.forEach(t => {
            ...
            if (mapped) {
                if (tpCountMap[mapped.id]) mapped.players = new Array(tpCountMap[mapped.id]);
...
async function _refreshTournamentsListOnly() {
    try {
        ...
        tournamentsRaw.forEach(t => {
            ...
            if (m) {
                if (tpCountMap[m.id]) m.players = new Array(tpCountMap[m.id]);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        players = playersRaw.map(mapPlayerFromDB).filter(p => p !== null);
        games = gamesRaw.map(mapGameFromDB).filter(g => g !== null);
        console.log('[BCC] Data loaded - players:', players.length, 'games:', games.length, 'tournaments:', tournamentsRaw.length);
        tournamentsRaw.forEach(t => {
            const mapped = mapTournamentFromDB(t);
            if (mapped) {
                if (tpCountMap[mapped.id]) mapped.players = new Array(tpCountMap[mapped.id]);
    ...
    api.subscribeToAllTables({
        ...
        onPairings: (payload) => {
            // A round was synced to DB by the admin device.
            // Only re-fetch for viewers (admin has _localTournament and ignores DB reads for it).
            ...
            if (window._localTournament && window._localTournament.id === tournamentId) return;
            // Viewer: re-open tournament detail from DB to show the new round
...
async function confirmGameSubmit() {
    ...
    try {
        ...
        // Update player ratings in DB — pass id + stats object separately
        await Promise.all([
...
function renderTournamentCard(tournament) {
    ...
    const formatLabel = tournament.format === 'swiss' ? 'Swiss' : tournament.format === 'roundrobin' ? 'Round Robin' : 'Knockout';
...
async function confirmStartTournament() {
    ...
    try {
        ...
        // Sync round 1 to DB so viewers can see pairings immediately.
        // Hide loading modal first so any error doesn't get masked by the 15s timeout.
        hideLoadingModal();
        try {
            await api.syncRoundToDb(saved.id, 1, round1PairingRows, round1Standings);
            console.log('[Start] Round 1 synced to DB successfully');
        } catch (e) {
            // Non-critical: admin can continue, viewers may need to refresh once
...
async function openTournamentDetail(tournamentId, forceTab = null, forceRound = null) {
    ...
    if (isCompleted) {
        ...
        try {
            ...
            if (rawPairings && rawPairings.length > 0) {
                // Resolve player names from tournament_players — same as completed path
                currentTournament.pairings = rawPairings.map(p => {
                    const wp = (currentTournament.players || []).find(x => x.id === p.white);
                    const bp = (currentTournament.players || []).find(x => x.id === p.black);
                    // Fall back to global players array if not in tournament roster
                ...
                // No pairings yet — tournament just started, pairings not yet synced
            ...
            <div class="tournament-detail-view active" >
                    ...
                    <div class="tournament-detail-header">
                        ...
                        <div style="display: flex; gap: 10px;">
                            ...
                            ${isAdmin && currentTournament.status?.toLowerCase() === 'active' && roundsPlayed < (currentTournament.total_rounds || currentTournament.totalRounds || 99) ? `<button class="btn-primary" onclick="generateNextRound()">Next Round</button>` : ''}
...
async function confirmRoundSubmit() {
    ...
    try {
        ...
        if (isLastRound) {
            ...
            // ── Push completed round to DB in background so viewers see it ──
            const tid = local.id;
            ...
            // Sync the just-completed round (with results) to DB
            api.syncRoundToDb(tid, currentRound, completedPairings, liveStandings)
                .then(() => {
                    // After completed round is saved, also push next round's pairings
                    // so viewers immediately see who plays in the next round
                    const nextRound = currentRound + 1;
                    const nextRoundPairings = (window._localTournament?.pairings || [])
                        .filter(p => p.round === nextRound);
                    if (nextRoundPairings.length > 0) {
                        const nextPairingRows = nextRoundPairings.map(p => ({
                            tournament_id: tid,
                            white_player_id: p.white,
                            black_player_id: p.isBye ? null : p.black,
                            result: null,
                            white_rating_before: p.whiteRatingBefore || window._localTournament?.players?.find(x => x.id === p.white)?.currentRating || 1600,
                            black_rating_before: p.isBye ? null : (p.blackRatingBefore || window._localTournament?.players?.find(x => x.id === p.black)?.currentRating || 1600),
                            white_rating_after: null,
                            black_rating_after: null,
                            white_rating_change: null,
                            black_rating_change: null,
                        ...
                        return api.syncRoundToDb(tid, nextRound, nextPairingRows, liveStandings);
                    }
                })
                .catch(e => console.warn('[Sync] Background round sync failed (non-critical):', e));
        }
...
async function _syncTournamentToSupabase(local) {
    ...
    // ── STEP 3: Insert pairings (idempotent) ─────────────────────────────────
    showLoadingModal('Step 3/8: Saving pairings...');
    let savedPairings = [];
    const pairingsExist = await api.checkPairingsExistForTournament(tid);
    if (!pairingsExist) {
        ...
        console.log('[Sync] Pairings already exist, skipping');
        savedPairings = await api.fetchTournamentPairings(tid);
    }

    // ── STEP 4: Insert games (idempotent) ────────────────────────────────────
    showLoadingModal('Step 4/8: Saving game records...');
    const gamesExist = await api.checkGamesExistForTournament(tid);
    let insertedGames = [];
    if (!gamesExist) {
    ...
    insertedGames.forEach(g => {
        gameIdMap[g.round_number + '|' + g.white_player_id] = g.id;
    });
    const ratingHistoryRows = [];
    const now = new Date().toISOString();
    for (const p of (local.pairings || []).filter(x => !x.isBye && x.result)) {
        const gameId = gameIdMap[p.round + '|' + p.white] || null;
    ...
    // ── STEP 7: Update global player stats from pairings table (source of truth) ──
    // Pairings covers ALL tournaments — even those where games table was empty
    showLoadingModal('Step 7/8: Updating player ratings...');
    const playerIds = local.players.map(p => p.id);
    const allPairings = await api.fetchPairingResultsForPlayers(playerIds);
    for (const p of local.players) {
        const newRating = p.currentRating || p.ratingAtStart || 1600;
        const myGames = allPairings.filter(g => g.white_player_id === p.id || g.black_player_id === p.id);
    ...
    if (h2hEntries.length > 0) {
        ...
        if (toUpdate.length > 0) {
            await Promise.all(toUpdate.map(u => api.updateH2HRow(u.player1_id, u.player2_id, {
                ...
                draws: u.draws, total_games: u.total_games
            })));
        }
    }
...
function openFirstActiveTournament() {
    // Find the best tournament to open — local first, then DB active
    const local = window._localTournament && !window._localTournament.synced ? window._localTournament : null;
    ...
    // Open the tournament detail modal
    const modal = document.getElementById('tournamentDetailModal');
    if (modal) modal.classList.add('active');
