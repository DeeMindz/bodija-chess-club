...
function saveLocalTournament() {
    if (!_localTournament) return;
    _localTournament.lastUpdated = Date.now();
    localStorage.setItem('bcc_active_tournament', JSON.stringify(_localTournament));
}
...
async function confirmStartTournament() {
    ...
    try {
        ...
        // Helper function to save local tournament state
        function saveLocalTournament() {
        ...
        // Make exactly ONE Supabase call to reserve the tournament ID
        ...
        // Update local state with the saved tournament ID
        localTournament.id = saved.id;
        ...
        await api.addRoundPairings(saved.id, pairingsWithIds);
