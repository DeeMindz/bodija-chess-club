...
function renderStats() {
    ...
    // Show blinking dot on Active Tournaments stat when there's a live tournament
    const activeCount2 = extendedTournaments.filter(t => t.status?.toLowerCase() === 'active').length
        + (window._localTournament && !window._localTournament.synced ? 1 : 0);
    const dot = document.getElementById('activeTournamentDot');
    if (dot) dot.style.display = activeCount2 > 0 ? 'inline-block' : 'none';
    const statItem = document.getElementById('activeTournamentStatItem');
    if (statItem) statItem.style.pointerEvents = activeCount2 > 0 ? 'auto' : 'none';
}
