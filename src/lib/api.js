...
export async function fetchAllPlayersForRecalc() {
    ...
    const { data, error } = await supabase
        .from('players')
        .select('id, name, bodija_rating, peak_rating, rapid_rating, rapid_peak_rating, blitz_rating, blitz_peak_rating, classical_rating, classical_peak_rating');
    if (error) throw error;
    return data || [];
}
...
export async function createPlayer(player) {
    ...
    const dbPlayer = {
        ...
        rapid_rating: player.rapid_rating || 1600,
        rapid_peak_rating: player.rapid_peak_rating || 1600,
        blitz_rating: player.blitz_rating || 1600,
        blitz_peak_rating: player.blitz_peak_rating || 1600,
        classical_rating: player.classical_rating || 1600,
        classical_peak_rating: player.classical_peak_rating || 1600,
...
export async function updatePlayerStats(id, stats) {
    guard();
    // Only pass keys that exist in stats object to avoid overwriting with undefined
    const payload = {};
    const allowedKeys = ['bodija_rating', 'peak_rating', 'rapid_rating', 'rapid_peak_rating', 'blitz_rating', 'blitz_peak_rating', 'classical_rating', 'classical_peak_rating', 'games_played', 'wins', 'draws', 'losses'];
    for (const key of allowedKeys) {
        if (stats[key] !== undefined) payload[key] = stats[key];
    }
