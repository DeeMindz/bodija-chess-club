...
window.showLoadingModal = utils.showLoadingModal;
window.hideLoadingModal = utils.hideLoadingModal;
...
window.getCategoryStats = ratings.getCategoryStats;
window.getRatingForCategory = ratings.getRatingForCategory;
window.getPeakRatingForCategory = ratings.getPeakRatingForCategory;
window.getPerformanceDataForCategory = ratings.getPerformanceDataForCategory;
window.deriveCategoryFromTimeControl = ratings.deriveCategoryFromTimeControl;
...
window.setLeaderboardCategory = function (cat) {
  ...
  document.querySelectorAll('.category-tab').forEach(tab => {
    if (tab.dataset.category === cat) tab.classList.add('active');else tab.classList.remove('active');
  });
...
// { playerId -> medals[] } — populated on load, used for inline rendering
...
window.recalcStats = async function () {
  console.log('Recalculating player stats from pairings table (source of truth)...');
  try {
    // Use games table — covers tournament games AND casual game-log games
    const [allGames, allPlayers] = await Promise.all([api.fetchAllGameResults(), api.fetchAllPlayersForRecalc()]);
    console.log(`Found ${allGames.length} games, ${allPlayers.length} players`);
    for (const player of allPlayers) {
      ...
      try {
        await api.updatePlayerStats(player.id, {
          bodija_rating: player.bodija_rating,
        ...
        console.log(`${player.name}: ${myGames.length} games, ${wins}W/${draws}D/${losses}L`);
      } catch (e) {
        console.warn('Failed for', player.name, e.message);
      }
    }
    console.log('Done. Refresh the page to see updated stats.');
  } catch (e) {
    console.error('recalcStats failed:', e);
  }
};
...
window.discardLocalTournament = async function () {
  ...
  if (local?.id) {
    try {
      await api.deleteTournament(local.id);
    } catch (e) {}
  }
...
window.submitPlayerRequest = async function (event) {
  ...
  try {
    const {
      ...
      error: checkError
    } = await supabase.from('player_requests').select('id, email, status').eq('email', email.toLowerCase()).in('status', ['pending', 'approved']);
...
async function loadPlayerRequests() {
  ...
  try {
    ...
    try {
      await api.deleteRejectedPlayerRequests();
    } catch (e) {}
    let data = [];
    try {
    ...
    if (container) container.innerHTML = pendingPlayerRequests.map(request => `
            <div class="player-request-card">
                ...
                <div class="player-request-info">
                    ${request.photo ? `<img src="${request.photo}" alt="Photo" class="player-request-photo">` : `<div class="player-request-photo-placeholder"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg></div>`}
...
window.bulkApproveRequests = async function () {
  ...
  store.players = (dbPlayers || []).map(mapPlayerFromDB);
  renderPlayers();
  renderLeaderboard();
  renderDashboard();
};
...
async function loadPendingRequestCount() {
  ...
  try {
    const {
      count,
      error
    } = await supabase.from('player_requests').select('*', {
      count: 'exact',
      head: true
    }).eq('status', 'pending');
    if (!error) {
...
async function silentRefresh() {
  try {
    const [playersRaw, gamesRaw, tournamentsRaw, tpCountMap] = await Promise.all([api.fetchPlayers(), api.fetchGames(), api.fetchTournaments(), api.fetchTournamentPlayerCounts()]);
    ...
    tournamentsRaw.forEach(t => {
      ...
      if (mapped) {
        if (tpCountMap[mapped.id]) {
          mapped.players = new Array(tpCountMap[mapped.id]);
          mapped.playerCount = tpCountMap[mapped.id];
        }
    ...
    try {
      store.medalsCache = await api.fetchAllPlayersMedals();
    } catch (e) {
      console.warn('Medals cache refresh failed:', e);
    }
    try {
      renderDashboard();
    } catch (e) {}
...
async function _refreshTournamentsListOnly() {
  try {
    const [tournamentsRaw, tpCountMap] = await Promise.all([api.fetchTournaments(), api.fetchTournamentPlayerCounts()]);
    extendedTournaments.length = 0;
    tournamentsRaw.forEach(t => {
      const m = mapTournamentFromDB(t);
      if (m) {
        if (tpCountMap[m.id]) {
          m.players = new Array(tpCountMap[m.id]);
          m.playerCount = tpCountMap[m.id];
        }
...
function sanitizeError(msg) {
  ...
  return String(msg)
  // Service names
  .replace(/supabase/gi, 'the server').replace(/vercel/gi, 'the server').replace(/postgres/gi, 'the database').replace(/postgresql/gi, 'the database')
  // URLs that might leak hostnames
  .replace(/https?:\/\/[^\s"')]+/gi, '[server]')
  // Supabase-style error codes / JWT noise
  .replace(/JWT\s+\w+/gi, 'session error').replace(/PGRST\d+/gi, 'server error')
...
function mapTournamentFromDB(dbTournament) {
  ...
  return {
    ...
    playerCount: 0,
    // reliable count — set separately from countMap
    pairings: [],
    standings: [],
    results: []
  };
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  Promise.all([api.fetchTournaments(), api.fetchTournamentPlayerCounts()]).then(([tournamentsRaw, tpCountMap]) => {
    tournamentsRaw.forEach(t => {
      const mapped = mapTournamentFromDB(t);
      if (mapped) {
        if (tpCountMap[mapped.id]) {
    ...
    try {
      ...
      console.error('renderPlayers failed:', e);
    }
    try {
      renderGamesLog();
    } catch (e) {
      console.error('renderGamesLog failed:', e);
    }
    try {
      populateH2HSelects();
    } catch (e) {
      console.error('populateH2HSelects failed:', e);
    }
    try {
      renderTournaments();
      _fadeIn('tournamentsGrid');
    } catch (e) {
      console.error('renderTournaments failed:', e);
    }

    // Medals cache (non-blocking, triggers re-render of leaderboard/players when ready)
    api.fetchAllPlayersMedals().then(m => {
      store.medalsCache = m;
      try {
        renderLeaderboard();
        renderPlayers();
  ...
  api.subscribeToAllTables({
    onPlayers: payload => {
      api.fetchPlayers().then(fresh => {
        if (fresh && fresh.length) {
          store.players = fresh.map(mapPlayerFromDB);
          renderLeaderboard();
          renderPlayers();
        }
    ...
    onPairings: payload => {
      ...
      if (currentTournament && currentTournament.id === tournamentId) {
        openTournamentDetail(tournamentId, currentTournamentTab || 'pairings', currentViewingRound).catch(e => console.warn('Realtime pairing refresh failed:', e));
      }
    },
...
function calculateNewRating(playerRating, opponentRating, actualScore) {
  ...
  const kFactor = playerRating < 1600 ? 40 : store.games.filter(g => g.white === player.id || g.black === player.id).length < 15 ? 40 : 20;
...
function renderPodium() {
  ...
  if (podium) podium.innerHTML = order.map((idx, displayIdx) => {
    ...
    if (!player) return '';
    const avatarContent = player.photo ? `<img src="${player.photo}" alt="${player.name}" class="podium-avatar-img">` : `<div class="podium-avatar ${medalClasses[displayIdx]}">${getInitials(player.name)}</div>`;
    return `
...
function renderStats() {
  ...
  if (totalGames) totalGames.textContent = store.games.length;
...
function _renderLiveTournamentBanner() {
  ...
  const progress = t.total_rounds > 0 ? Math.round(((t.current_round || 1) - 1) / t.total_rounds * 100) : 0;
  const playerCount = t.players?.length || 0;

  // onclick — works on mobile too (no hover events needed)
  const clickFn = local ? `openFirstActiveTournament()` : `openTournamentDetail('${t.id}')`;
...
function renderRecentGames() {
  ...
  const recentGames = [...store.games].sort((a, b) => new Date(b.created_at || b.date).getTime() - new Date(a.created_at || a.date).getTime() || (b.round || 0) - (a.round || 0)).slice(0, 10);
...
window.adminUpdatePlayerPhoto = async function (playerId, inputEl) {
  ...
  try {
    ...
    if (idx !== -1) store.players[idx].photo = publicUrl;
    hideLoadingModal();
...
async function openMedalsModal(playerId) {
  ...
  const emptyHtml = _currentMedals.length === 0 ? '<div style="text-align:center;padding:32px;color:var(--text-secondary);opacity:0.55;font-size:14px;">No medals yet</div>' : '';
  ...
  if (!modal) {
    ...
    modal.onclick = e => {
      if (e.target === modal) closeMedalsModal();
    };
...
function populateTournamentFilter() {
  ...
  const tourns = [...new Set(store.games.map(g => g.tournament).filter(t => t))];
  if (select) select.innerHTML = '<option value="">All Tournaments</option>' + tourns.map(t => `<option value = "${t}" > ${t}</option > `).join('');
}
...
async function confirmGameSubmit() {
  ...
  try {
    ...
    // Update players
    const whiteIdx = store.players.findIndex(p => p.id === whiteId);
    const blackIdx = store.players.findIndex(p => p.id === blackId);
    if (category === 'rapid') {
      store.players[whiteIdx].rapid_rating = (store.players[whiteIdx].rapid_rating || store.players[whiteIdx].bodija_rating || 1600) + whiteChange;
      store.players[whiteIdx].bodija_rating = store.players[whiteIdx].rapid_rating;
    } else if (category === 'blitz') store.players[whiteIdx].blitz_rating = (store.players[whiteIdx].blitz_rating || 1600) + whiteChange;else if (category === 'classical') store.players[whiteIdx].classical_rating = (store.players[whiteIdx].classical_rating || 1600) + whiteChange;
    store.players[whiteIdx].rating = getRatingForCategory(store.players[whiteIdx], window.activeLeaderboardCategory);
    if (category === 'rapid') {
      store.players[blackIdx].rapid_rating = (store.players[blackIdx].rapid_rating || store.players[blackIdx].bodija_rating || 1600) + blackChange;
      store.players[blackIdx].bodija_rating = store.players[blackIdx].rapid_rating;
    } else if (category === 'blitz') store.players[blackIdx].blitz_rating = (store.players[blackIdx].blitz_rating || 1600) + blackChange;else if (category === 'classical') store.players[blackIdx].classical_rating = (store.players[blackIdx].classical_rating || 1600) + blackChange;
    store.players[blackIdx].rating = getRatingForCategory(store.players[blackIdx], window.activeLeaderboardCategory);
    store.players[whiteIdx].games++;
    store.players[blackIdx].games++;
    if (result === '1-0') {
      store.players[whiteIdx].wins++;
      store.players[blackIdx].losses++;
    } else if (result === '0-1') {
      store.players[whiteIdx].losses++;
      store.players[blackIdx].wins++;
    } else {
      store.players[whiteIdx].draws++;
      store.players[blackIdx].draws++;
    }

    // Update peak ratings

    let wRating = getRatingForCategory(store.players[whiteIdx], category);
    let wPeak = getPeakRatingForCategory(store.players[whiteIdx], category);
    if (wRating > wPeak) {
      if (category === 'rapid') {
        store.players[whiteIdx].rapid_peak_rating = wRating;
        store.players[whiteIdx].peakRating = wRating;
      }
      if (category === 'blitz') store.players[whiteIdx].blitz_peak_rating = wRating;
      if (category === 'classical') store.players[whiteIdx].classical_peak_rating = wRating;
    }
    // block old peak updating
    if (false) {
      store.players[whiteIdx].peakRating = store.players[whiteIdx].rating;
    }
    let bRating = getRatingForCategory(store.players[blackIdx], category);
    let bPeak = getPeakRatingForCategory(store.players[blackIdx], category);
    if (bRating > bPeak) {
      if (category === 'rapid') {
        store.players[blackIdx].rapid_peak_rating = bRating;
        store.players[blackIdx].peakRating = bRating;
      }
      if (category === 'blitz') store.players[blackIdx].blitz_peak_rating = bRating;
      if (category === 'classical') store.players[blackIdx].classical_peak_rating = bRating;
    }
    // block old peak updating
    if (false) {
      store.players[blackIdx].peakRating = store.players[blackIdx].rating;
    }

    // Add rating to history
    store.players[whiteIdx].ratingHistory.push(store.players[whiteIdx].rating);
    store.players[blackIdx].ratingHistory.push(store.players[blackIdx].rating);

    // Add game to log
    store.games.push(newGame);

    // Sync with database
    const updatedWhitePlayer = store.players[whiteIdx];
    const updatedBlackPlayer = store.players[blackIdx];
...
function renderTournamentCard(tournament) {
  ...
  const onclick = status === 'draft' ? `editTournament('${tournament?.id ?? ''}')` : `openTournamentDetail('${tournament?.id ?? ''}')`;

  // Status dot — only active tournament gets a blinking green dot
  const dotHtml = status === 'active' ? `<span class="t-card-dot" style="background:#22c55e;animation:blink 1.2s infinite;flex-shrink:0;margin-top:4px;"></span>` : '';
...
function showTournamentStartPreview() {
  ...
  if (normFmt === 'roundrobin') {
    ...
    const _swissRounds = currentTournament.total_rounds ?? currentTournament.totalRounds ?? currentTournament.rounds ?? null;
    expectedRounds = _swissRounds && _swissRounds > 0 ? _swissRounds : Math.ceil(Math.log2(n));
...
async function confirmStartTournament() {
  ...
  try {
    ...
    if (normalizeFormat(_format) === 'roundrobin') {
      ...
      const manualRounds = currentTournament.total_rounds ?? currentTournament.totalRounds ?? currentTournament.rounds ?? null;
      _totalRounds = manualRounds && manualRounds > 0 ? manualRounds : Math.ceil(Math.log2(_playerCount));
    }
    ...
    const round1Standings = window._localTournament.players.map(p => ({
      ...
      losses: 0,
      byes: 0,
      rating_at_start: p.ratingAtStart || 1600,
      rating_change: 0,
      buchholz: 0
    }));
    hideLoadingModal();
...
async function submitTournament(e) {
  ...
  try {
    ...
    if (editId) {
      ...
      const existingDraft = extendedTournaments.find(t => t.status?.toLowerCase() === 'draft' && t.name === name && t.date === date);
      if (existingDraft) {
...
async function deleteTournament(id) {
  ...
  if (tournament.status?.toLowerCase() === 'active') {
    showConfirmModal('Terminate Tournament', 'This tournament is currently ACTIVE. Terminating it will DELETE all games played in this tournament, RESET all players to their ratings before the tournament started, and DELETE the tournament record. Are you sure you want to proceed?', async () => {
      try {
  ...
  showConfirmModal('Delete Tournament', 'Are you sure you want to delete this tournament? This action cannot be undone.', async () => {
    try {
...
function renderPlayerCards() {
  ...
  const filteredPlayers = store.players.filter(p => p.name.toLowerCase().includes(search));
...
async function addNewPlayer() {
  ...
  try {
    ...
    if (newPlayer) {
      ...
      api.fetchPlayers().then(fresh => {
        store.players = fresh.map(mapPlayerFromDB).filter(p => p !== null);
        renderPlayers();
        renderLeaderboard();
    ...
    if (round1) {
      ...
      if (dbPairings && Array.isArray(dbPairings)) {
        ...
        currentTournament.pairings = dbPairings.map(dbp => {
          const local = currentTournament.pairings.find(lp => lp.white === dbp.white_player_id && lp.black === dbp.black_player_id);
          return {
            ...local,
            id: dbp.id,
            roundId: dbp.round_id
          };
        });
      }
    }
...
function assignColors(p1, p2, colorHistory) {
  const h1 = colorHistory[p1.id] || {
    whites: 0,
    blacks: 0,
    lastColor: null,
    streak: 0
  };
  const h2 = colorHistory[p2.id] || {
    whites: 0,
    blacks: 0,
    lastColor: null,
    streak: 0
  ...
  if (h1.lastColor === 'white' && h1.streak >= 2) return {
    white: p2,
    black: p1
  };
  if (h2.lastColor === 'white' && h2.streak >= 2) return {
    white: p1,
    black: p2
  };
  if (h1.lastColor === 'black' && h1.streak >= 2) return {
    white: p1,
    black: p2
  };
  if (h2.lastColor === 'black' && h2.streak >= 2) return {
    white: p2,
    black: p1
  };
  ...
  if (diff2 < diff1) return {
    white: p2,
    black: p1
  };

  // Alternation: player who had black last gets white
  if (h1.lastColor === 'black') return {
    white: p1,
    black: p2
  };
  if (h2.lastColor === 'black') return {
    white: p2,
    black: p1
  };
  return {
...
function generatePairings() {
  ...
  const playersLocal = [...currentTournament.players];
  let pairings = [];
  if (format === 'swiss') pairings = generateSwissPairings(playersLocal, currentTournament.current_round || 1);else if (format === 'roundrobin') pairings = generateRoundRobinPairings(playersLocal, currentTournament.current_round || 1);else if (format === 'knockout') pairings = generateKnockoutPairings(playersLocal, currentTournament.current_round || 1);
...
function generateRoundLocally() {
  ...
  const playersLocal = currentTournament.players.map(p => ({
    ...p,
    rating: p.currentRating || p.ratingAtStart || p.rating || 1600
  }));
  let rawPairings = [];
  if (format === 'swiss') rawPairings = generateSwissPairings(playersLocal, currentRound);else if (format === 'roundrobin') rawPairings = generateRoundRobinPairings(playersLocal, currentRound);else if (format === 'knockout') rawPairings = generateKnockoutPairings(playersLocal, currentRound);else console.warn('Unknown format:', currentTournament.format);
...
function generateRoundRobinPairings(playersLocal, round) {
  ...
  for (let i = 0; i < N / 2; i++) {
    ...
    if (isSecondHalf) {
      ...
      const mirror = firstHalfPairings.find(p => p.white === top.id && p.black === bottom.id || p.white === bottom.id && p.black === top.id);
      if (mirror) {
      ...
      const topH = colorHistory[top.id] || {
        streak: 0,
        lastColor: null
      };
      const botH = colorHistory[bottom.id] || {
        streak: 0,
        lastColor: null
      };
...
function generateSwissPairings(playersLocal, round) {
  const colorHistory = getColorHistory(currentTournament);
  const pairings = [];
  ...
  if (sorted.length % 2 !== 0) {
    ...
    for (let i = sorted.length - 1; i >= 0; i--) {
      if (!sorted[i].byes || sorted[i].byes === 0) {
        byePlayer = sorted[i];
        break;
      }
    }
    ...
    for (let ti = 0; ti < top.length; ti++) {
      ...
      for (let bi = 0; bi < bottom.length; bi++) {
        ...
        const {
          white,
          black
        } = assignColors(p1, p2, colorHistory);
        pairings.push({
          round,
          white: white.id,
          whiteName: white.name,
...
function generateKnockoutPairings(playersLocal, round) {
  const colorHistory = getColorHistory(currentTournament);
  const pairings = [];
  if (round === 1) {
    ...
    for (let i = 0; i < top.length; i++) {
      ...
      if (!p2) {
        pairings.push({
          ...
          result: '1-0',
          isBye: true,
          whiteRatingBefore: p1.currentRating || p1.rating || 0,
          blackRatingBefore: null
        });
        continue;
      }
    ...
    const winners = prevPairings.map(p => {
      if (p.isBye) return store.players.find(x => x.id === p.white);
      if (!p.result) return null;
      const wid = p.result === '1-0' ? p.white : p.result === '0-1' ? p.black : p.white;
      return store.players.find(x => x.id === wid);
    }).filter(Boolean);
    ...
    for (let i = 0; i < top.length; i++) {
      ...
      if (prevGames.length > 0) {
        ...
        const a = assignColors(p1, p2, colorHistory);
        white = a.white;
        black = a.black;
      }
      pairings.push({
...
async function openTournamentDetail(tournamentId, forceTab = null, forceRound = null) {
  ...
  if (isCompleted) {
    ...
    try {
      const [tpPlayers, rawPairings] = await Promise.all([api.fetchTournamentStandings(tournamentId), api.fetchTournamentPairings(tournamentId)]);
    ...
    try {
      ...
      if (rawPairings && rawPairings.length > 0) {
        ...
        currentTournament.pairings = rawPairings.map(p => {
          const wp = (currentTournament.players || []).find(x => x.id === p.white);
          const bp = (currentTournament.players || []).find(x => x.id === p.black);
          const wg = store.players.find(x => x.id === p.white);
          const bg = store.players.find(x => x.id === p.black);
          return {
            ...p,
                    ...
                    <div class="tournament-detail-header">
                        <div>
                            ...
                            <div class="tournament-detail-meta">
                                <span>&#x1F4C5; ${new Date(currentTournament.date).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'long',
    day: 'numeric'
  })}</span>
...
function renderTournamentTab() {
  switch (currentTournamentTab) {
    ...
    case 'pairings':
      return renderTournamentPairings();
    case 'standings':
      return renderTournamentStandings();
    case 'bracket':
...
function renderTournamentFinalSummary() {
  ...
  const tournamentDateStr = currentTournament.date ? new Date(currentTournament.date).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'short',
    day: 'numeric'
  }) : '';
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
            ...
            ${isCurrentActiveRound ? `
                <div style="font-size: 14px; color: ${resultsRemaining === 0 ? 'var(--success)' : 'var(--text-secondary)'};">
                    ${resultsRemaining === 0 ? '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:4px;"><polyline points="20 6 9 17 4 12"></polyline></svg>All results entered' : `${resultsRemaining} result${resultsRemaining === 1 ? '' : 's'} remaining`}
                </div>
            ` : ''}
        </div>
...
function recordResult(pairingId, result) {
  ...
  const pairing = local.pairings ? local.pairings.find(p => p.id === pairingId || local.pairings.indexOf(p).toString() === pairingId) : null;
  if (!pairing) return;
...
async function confirmRoundSubmit() {
  ...
  try {
    if (btn) {
      btn.disabled = true;
      btn.textContent = isLastRound ? 'Finishing...' : 'Confirming...';
    }
    ...
    const _localForRecalc = {
      ...local,
      rounds: (local.rounds || []).map(r => ({
        ...r,
    ...
    const completedRoundNumbers = [...new Set((local.pairings || []).filter(p => p.result || p.isBye).map(p => p.round))];
    ...
    for (const rp of _pairingsForRecalc) {
      const lp = (local.pairings || []).find(p => p.round === rp.round && p.white === rp.white && p.black === rp.black);
      if (!lp) continue;
    ...
    if (isLastRound) {
      ...
      try {
        ...
        console.warn('Post-finish refresh failed:', e);
      }
      ...
      api.syncRoundToDb(tid, currentRound, completedPairings, liveStandings).then(() => {
  ...
  try {
    ...
    try {
      ...
      for (const p of (local.pairings || []).filter(x => !x.isBye && x.result)) {
        ...
        if (p.result === '1/2-1/2') h2hMap[key].draws++;else if (p.result === (p1IsWhite ? '1-0' : '0-1')) h2hMap[key].player1_wins++;else h2hMap[key].player2_wins++;
      }
      ...
      if (h2hEntries.length > 0) {
        ...
        if (toUpdate.length > 0) {
          await Promise.all(toUpdate.map(u => api.updateH2HRow(u.player1_id, u.player2_id, {
            player1_wins: u.player1_wins,
            player2_wins: u.player2_wins,
            draws: u.draws
          })));
        }
      }
    } catch (h2hErr) {
      console.warn('[Sync] H2H step failed (non-critical):', h2hErr.message);
    }
...
async function closeTournament() {
  if (!currentTournament) return;
  showConfirmModal('End Tournament Early', 'Are you sure you want to end this tournament early? Final standings will be locked and the winner will be declared based on current points.', async () => {
    try {
...
window.triggerPwaInstall = async function () {
  ...
  if (_pwaInstallPrompt) {
    ...
    const {
      outcome
    } = await _pwaInstallPrompt.userChoice;
    if (outcome === 'accepted') {
      _pwaInstallPrompt = null;
      _hideInstallBtn();
    }
...
window.repairPlayerRatings = async function () {
  ...
  try {
    ...
    const {
      data: allGames,
      error: gErr
    } = await supabase.from('games').select('id, white_player_id, black_player_id, result, date, round_number, created_at').order('date', {
      ascending: true
    }).order('round_number', {
      ascending: true
    }).order('created_at', {
    ...
    for (const g of allGames || []) {
      ...
      if (statsMap[wId]) {
        statsMap[wId].games++;
        if (g.result === '1-0') statsMap[wId].wins++;else if (g.result === '1/2-1/2') statsMap[wId].draws++;else statsMap[wId].losses++;
      }
      if (statsMap[bId]) {
        statsMap[bId].games++;
        if (g.result === '0-1') statsMap[bId].wins++;else if (g.result === '1/2-1/2') statsMap[bId].draws++;else statsMap[bId].losses++;
      }
      gameUpdates.push({
        id: g.id,
...
function renderGamesLog() {
  ...
  let filtered = [...store.games].sort((a, b) => new Date(b.created_at || b.date).getTime() - new Date(a.created_at || a.date).getTime() || (b.round || 0) - (a.round || 0));
  if (tournamentFilter) {
